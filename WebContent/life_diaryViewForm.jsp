...
<body>
	<%
		...
		Diary diary = new Life_DiaryDAO().getDiary(ID, userID);
	%>
	...
	<div class="joinForm_div">
		...
		<div class="m">
			<a class="m_a" href="life_diary.jsp">목록</a>
		<%
			if (userID != null) {
		%>
		<a class="m_a" href="life_diaryUpdate.jsp?ID=<%=ID%>">수정</a> 
		<a class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')" href="life_diaryDelete.jsp?ID=<%=ID%>">삭제</a>
		<%
			}
		%>
		<a class="m_a" href="life_diaryWrite.jsp">글쓰기</a>
		</div>
		...
		<table class="coment_t2">
			<tbody>
				<%
					...
					ArrayList<Comment> list = life_commDAO.getList(commNum, diary.getID());
				...
				<tr>
					...
					<a  class="m_a" onclick="location.href='life_commUpdateForm.jsp?ID=<%=list.get(i).getID()%>'">수정</a>
					<a  class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')"
						href='life_commDelete.jsp?ID=<%=list.get(i).getID()%>'>삭제</a>
					</td>
				</tr>
				<%
					}
		...
		<form method="post" action="life_commProc.jsp">
