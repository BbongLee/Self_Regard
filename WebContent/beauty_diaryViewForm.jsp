...
<body>
	<%
		...
		Diary diary = new Beauty_DiaryDAO().getDiary(ID, userID);
	%>
	...
	<div class="joinForm_div">
		...
		<%
			if(userID != null){
		%>
			<a class="m_a" href="beauty_diaryUpdate.jsp?ID=<%= ID %>">수정</a>
			<a class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')" href="beauty_diaryDelete.jsp?ID=<%= ID %>">삭제</a>
		<%
			}
		%>
		...
		<table class="coment_t2">
			<tbody>
				<% //댓글!!!
					Beauty_CommDAO beauty_commDAO = new Beauty_CommDAO();
					ArrayList<Comment> list = beauty_commDAO.getList(commNum, diary.getID());
				...
				<tr>
					...
					<a  class="m_a" onclick="location.href='beauty_commUpdateForm.jsp?ID=<%=list.get(i).getID() %>'">수정</a>
					<a  class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')" href='beauty_commDelete.jsp?ID=<%=list.get(i).getID() %>'>삭제</a>
					</td>
				</tr>
				<%
					}
