...
<body>
	<%
		...
		Diary diary = new Family_DiaryDAO().getDiary(ID, userID);
	%>
	...
	<div class="joinForm_div">
		...
		<div class="m">
			<a class="m_a" href="family_diary.jsp">목록</a>
		<%
			if(userID != null){
		%>
			<a class="m_a" href="family_diaryUpdate.jsp?ID=<%= ID %>">수정</a>
			<a class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')" href="family_diaryDelete.jsp?ID=<%= ID %>">삭제</a>
		<%
			}
		%>
		<a class="m_a" href="family_diaryWrite.jsp">글쓰기</a>
		</div>
		
		<table class="coment_t2">
			<tbody>
				<% //댓글!!!
					Family_CommDAO family_commDAO = new Family_CommDAO();
					ArrayList<Comment> list = family_commDAO.getList(commNum, diary.getID());
					...
					for(int i = 0; i < list.size(); i++){
						...
						<tr>
							...
							<a  class="m_a" onclick="location.href='family_commUpdateForm.jsp?ID=<%=list.get(i).getID() %>'">수정</a>
							<a  class="m_a" onclick="return confirm('정말로 삭제하시겠습니까?')" href='family_commDelete.jsp?ID=<%=list.get(i).getID() %>'>삭제</a>
							</td>
						</tr>
				<%
					}
	...
	<form method="post" action="family_commProc.jsp">
